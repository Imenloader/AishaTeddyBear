...
# We find the flex-wrap div and add the Open When button to it.
open_when_btn = """
               <button
...
"""
content = content.replace('title="جلسة تسبيح"', 'title="جلسة تسبيح"\n               >\n                 <span className="text-xl">📿</span>\n               </button>' + open_when_btn)
# Actually, replacing like this is risky. Let's use regex to find `<div className="flex justify-center gap-3 w-full flex-wrap">`
# and insert the button right after it.
...
# The old Thought of you button was a single large button. I'll replace it with a flex row of 2 buttons.
...
daily_card_regex = re.compile(r'(\<motion\.div\s*key="daily".*?className=\`backdrop-blur-md rounded-2xl p-5 shadow-sm border border-white\/50 text-center)(.*?)(\<\/motion\.div\>)', re.DOTALL)
new_daily_card = r"""\1 relative\2
                  {/* Toggle Button */}
                  <button 
...
\3"""
content = daily_card_regex.sub(new_daily_card, content)

# Remove the old inner h3 and p inside daily card
# Wait, my regex captured `(.*?)` which INCLUDES the old `<h3...>{dailyMsg.title}</h3>` and `<p...>{dailyMsg.content}</p>`.
# If I just inject my toggle logic inside `\2`, the old static elements will duplicate!
# Let's fix the regex substitution.
"""
daily_card_inner = r'''
                  {/* Toggle Button */}
...
...
# Let's use string replace for the inner daily card instead!
...
# Also make the daily card relative
